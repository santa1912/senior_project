  ...
  <div class="min-h-screen flex">
    ...
    <aside class="fixed inset-y-0 left-0 bg-gradient-to-br from-[#046e93] via-[#035e80] to-[#1b3455] shadow-xl w-64 z-50 flex flex-col md:hidden" v-if="showSidebar" aria-label="Mobile sidebar" style="z-index:9999">
      ...
      <nav class="flex-1 px-4 py-6">
        <ul class="space-y-1">
          <li>
            <NuxtLink to="/lecturer/lecturer" @click="showSidebar = false" class="flex items-center px-4 py-3 text-white hover:bg-[#035e80] rounded-md transition-colors" :class="{ 'bg-[#035475]': $route.path === '/lecturer/lecturer' }">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
              </svg>
              Profile

  ...
  <div class="min-h-screen flex">
    ...
    <aside class="w-64 bg-gradient-to-b from-[#18345c] via-[#17688f] to-[#137b97] text-white flex-col sticky top-0 h-screen hidden md:flex">
      ...
      <nav class="flex-1 px-4 py-6">
        <ul class="space-y-1">
          <li>
            <NuxtLink to="/lecturer/profile" class="flex items-center px-4 py-3 text-white hover:bg-[#035e80] rounded-md transition-colors" :class="{ 'bg-[#035475]': $route.path === '/lecturer/profile' }">
    ...
    <aside class="fixed inset-y-0 left-0 bg-gradient-to-b from-[#18345c] via-[#17688f] to-[#1597bb] shadow-xl w-64 z-50 flex flex-col md:hidden" v-if="showSidebar" aria-label="Mobile sidebar" style="z-index:9999">
      ...
      <div class="px-8 py-6 border-b border-[#035e80] flex justify-between items-center">
        ...
        <button @click="showSidebar = false" class="text-white" aria-label="Close sidebar">
      ...
      <nav class="flex-1 px-4 py-6">
        <ul class="space-y-1">
          <li>
            <NuxtLink to="/lecturer/profile" @click="showSidebar = false" class="flex items-center px-4 py-3 text-white hover:bg-[#035e80] rounded-md transition-colors" :class="{ 'bg-[#035475]': $route.path === '/lecturer/profile' }">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
              </svg>
              Profile
      ...
      <!-- User Profile at Bottom -->
      <div class="p-4 border-t border-[#035e80] flex items-center mt-auto">
        <div class="w-10 h-10 rounded-full bg-white overflow-hidden mr-3">
          <img :src="user?.photoURL || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(user?.displayName || 'User')" :alt="user?.displayName || 'User'" class="w-full h-full object-cover" />
        </div>
        <div>
        ...
        <button @click="logout" class="ml-auto text-blue-300 hover:text-white" aria-label="Logout">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
          </svg>
        </button>
